...
function StatusProgress(props) {
  ...
  if (status === -3) {
    ...
    element = (<span>{total && total > 0 ? `${current ? `${current}` : '0'}/${total}` : ''}&nbsp;{progress}%</span>);
  } else if (status === 2) {
...
class TaskTable extends PureComponent {
  static propTypes = {
    ...
    onRowClick: func.isRequired, // 当前行点击事件
  ...
  handleUpdateStatusTask = item => {
    ...
    updateStatusTask(id, { type: TASK_TYPE.PIC, status: statusTemp === 1 ? 2 : 1 });
  }
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.formatMessage({ id: 'App.picture.source' }),
