...
import { Button, Modal, message, Card, Tag, Checkbox } from 'antd';
...
import TaskPicAdd from '../../../shared/containers/TaskVideoAdd';
...
class TaskPic extends PureComponent {
  static propTypes = {
  ...
  state = {
    ...
    modalVisibleResultList: false, // 任务结果界面
    modalVisibleCmodal: false,
    selectedRowKeys: [],
    page: 1, // 当前页
    pageSize: 14, // 当前页数量
    delSize: 0, // 一个用户删除个数，用来避免最后一页删除后，还是请求的没有用的
    deleteFile: 1, // eslint-disable-line react/no-unused-state
    currTask: {}, // 当前row的任务
  }
  ...
  handleFetchTasks = suppressLoading => {
    ...
    const filters = {
      page: pageTemp,
      pageSize,
      type: TASK_TYPE.PIC, // 任务类型（1实时，2视频，3图集）
      suppressLoading,
    };
  ...
  handleChange = e => {
    this.setState({
      deleteFile: e.target.checked ? 0 : 1, // eslint-disable-line react/no-unused-state
    });
  }
  ...
  handleDelete = id => {
    ...
    Modal.confirm({
      title,
      okType: 'danger',
      content: <Checkbox onChange={this.handleChange}>{intl.formatMessage({ id: 'App.picture.delete.warning' })}</Checkbox>,
      onOk() {
        const { deleteFile } = self.state;
        deleteTasks(id, [TASK_TYPE.PIC], deleteFile)
          .then(({ isOk }) => {
            if (isOk) {
              self.setState({
                selectedRowKeys: [],
  ...
  handleMultiStop = () => {
    ...
    updateStatusLotTask({ tasks: statusArr, type: TASK_TYPE.PIC })
      .then(({ isOk }) => {
        if (isOk) {
          self.setState({
            selectedRowKeys: [],
  ...
  handleMultiDelete = () => {
    ...
    deleteTasks(selectedRowKeys, [TASK_TYPE.PIC])
      .then(({ isOk }) => {
        if (isOk) {
          self.setState({
            selectedRowKeys: [],
        ...
        <div className="app-page">
          ...
          >
            <h1 className="app-page__title">
              {intl.formatMessage({ id: 'App.picture.title' })}
            </h1>
          ...
          {this.state.modalVisibleTaskAdd &&
            <Modal
              ...
              footer={null}
            >
              <TaskPicAdd hideModal={this.hideModalTaskAdd} taskType={TASK_TYPE.PIC} intl={intl} />
            </Modal>}
...
export default injectIntl(TaskPic);
